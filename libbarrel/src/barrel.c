...
BRL_Error BRL_WriteEx(BRL_Archive* arch, uint64_t hash, const void* data, uint64_t size, bool use_compressor) {
    ...
    arch->metadata[target_idx].flags           = is_compressed ? BRL_ENTRY_COMPRESSED : BRL_ENTRY_ACTIVE;
...
uint64_t BRL_HashString(const char* str) {
    ...
    // Prevent collision with EMPTY (0) and TOMBSTONE (1) hashes
    if (hash <= BRL_TOMBSTONE_HASH) hash = 2; 
    return hash;
}
