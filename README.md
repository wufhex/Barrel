...
`libbarrel` is the core library, it's written in standard C99 and works natively in C++. The API is also designed to make building bindings for other languages easy.    

Examples can be found in `test_suite`, `example`, `pybarrel_test`. Check `WIKI.md` to see all the features, understand how Barrel works and how to use it.
...
Barrel also supports Python via `pybarrel`, however the API is still experimental and still not available on PyPI. Some examples can be found in the `pybarrel_test` folder.

To manage Barrel archives quickly, you can use bartool (`bartool/`). To use it, you must install the requirements and `pybarrel`.

You can install both `pybarrel` and `bartool` by running this command in the root of the project:
...
* **Punch Hole Writing**: The underlying OS file system dynamically allocates physical blocks only when data is actively written to a slot, silently discarding unused ranges back to the disk. This doesn't apply if the file is shared or moved to another disk, for this purpose, use `BRL_Pack` to collapse all the null bytes. This isn't strictly required, but it's often useful. Check `WIKI.md` for a detailed explaination.
