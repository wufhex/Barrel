...
bool Test_PackAndUnpack() {
    const char* path = "./test_pack.brl";
    constexpr uint64_t VIRTUAL_CAP = 256 * 1024 * 1024; // 256 MiB

    std::error_code ec;
    std::filesystem::remove(path, ec);

    // Setup initial archive
    TEST_ASSERT(BRL_Create(path, 0, 512, VIRTUAL_CAP) == BRL_OK, "Failed to create pack test archive");

    BRL_Archive* arch = nullptr;
    TEST_ASSERT(BRL_Open(path, OpenFlagNormal, &arch) == BRL_OK, "Failed to open archive");
    BRL_SetCompressor(arch, &g_test_compressor);

    // Write active payloads separated by deletions (creates fragmentation & orphan holes)
    std::vector<uint8_t> buf1 = GenerateRandomBuffer(1024 * 1024, 111); // 1 MiB
    std::vector<uint8_t> buf2 = GenerateRandomBuffer(2048 * 1024, 222); // 2 MiB
    std::vector<uint8_t> buf3 = GenerateRandomBuffer(512 * 1024,  333); // 512 KiB

    uint64_t h1 = BRL_HashString("PACK_PAYLOAD_1");
    uint64_t h2 = BRL_HashString("PACK_PAYLOAD_2");
    uint64_t h3 = BRL_HashString("PACK_PAYLOAD_3");

    TEST_ASSERT(BRL_Write(arch, h1, buf1.data(), buf1.size()) == BRL_OK, "Write payload 1 failed");
    TEST_ASSERT(BRL_Write(arch, h2, buf2.data(), buf2.size()) == BRL_OK, "Write payload 2 failed");
    TEST_ASSERT(BRL_Delete(arch, h2) == BRL_OK, "Delete payload 2 failed"); // Hole created
    TEST_ASSERT(BRL_WriteEx(arch, h3, buf3.data(), buf3.size(), true) == BRL_OK, "Write compressed payload 3 failed");

    // Pack archive and verify physical size reduction
    uint64_t hwm_before_pack = arch->header->high_water_mark;
    TEST_ASSERT(BRL_Pack(arch) == BRL_OK, "BRL_Pack failed");

    uint64_t packed_size_on_disk = std::filesystem::file_size(path);
    TEST_ASSERT(packed_size_on_disk < hwm_before_pack, "Packed size was not reduced on disk");
    TEST_ASSERT((arch->header->flags & BRL_ARCHIVE_PACKED) != 0, "BRL_ARCHIVE_PACKED flag was not set");

    BRL_Close(arch);

    // Reopen packed archive (tests auto unpack in BRL_Open)
    TEST_ASSERT(BRL_Open(path, OpenFlagNormal, &arch) == BRL_OK, "Failed to open packed archive");
    BRL_SetCompressor(arch, &g_test_compressor);

    TEST_ASSERT((arch->header->flags & BRL_ARCHIVE_PACKED) == 0, "BRL_ARCHIVE_PACKED flag was not cleared post-open");
    TEST_ASSERT(arch->header->virtual_capacity == VIRTUAL_CAP, "Virtual capacity was not restored post-unpack");
    TEST_ASSERT(std::filesystem::file_size(path) == VIRTUAL_CAP, "Physical disk capacity was not extended post-unpack");

    // Verify integrity of remaining payloads post-unpack
    const uint8_t* read_ptr = nullptr;
    uint64_t read_sz = 0;
    TEST_ASSERT(BRL_Read(arch, h1, &read_ptr, &read_sz) == BRL_OK, "Failed to read payload 1 post-unpack");
    TEST_ASSERT(memcmp(read_ptr, buf1.data(), buf1.size()) == 0, "Payload 1 corrupted post-unpack");

    std::vector<uint8_t> decomp_buf(buf3.size());
    uint64_t decomp_written = 0;
    TEST_ASSERT(BRL_ReadCopy(arch, h3, decomp_buf.data(), decomp_buf.size(), &decomp_written) == BRL_OK, "Decompress payload 3 failed post-unpack");
    TEST_ASSERT(memcmp(decomp_buf.data(), buf3.data(), buf3.size()) == 0, "Payload 3 corrupted post-unpack");

    TEST_ASSERT(BRL_Read(arch, h2, &read_ptr, &read_sz) == BRL_ENTRY_NOT_FOUND, "Deleted payload 2 reappeared post-unpack");
